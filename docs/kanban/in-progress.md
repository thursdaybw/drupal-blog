...
- The cull/unpublish flow removes remote offers and inventory but leaves stale local mirror rows behind.
...
- [ ] `already unpublished` / missing remote SKU handling still classifies correctly and removes local mirror rows.
- [ ] Add or update tests covering both direct SKU deletion and cull/unpublish paths.
